(function () {
	angular.module('app').controller('servicesCtrl', function ($scope, $sce, $stateParams, $location, $anchorScroll, mainService) {
		...
		$scope.changeContent = function(num){
			...
			if(lastSection !== num){
				if(lastSection === null){
					tl.to(document.getElementById('overlay-' + num), 0.05, {height: 0})
						.to(document.getElementById('panel-' + num), 0.05, {"border-bottom": "13px solid #fff"})
						.to(document.getElementById('arrow-' + num), 0.05, {bottom: "15px"})
				} else {
					tl.to(document.getElementById('overlay-' + num), 0.05, {height: 0})
						.to(document.getElementById('panel-' + num), 0.05, {"border-bottom": "13px solid #fff"})
						.to(document.getElementById('arrow-' + num), 0.05, {bottom: "15px"})
						.to(document.getElementById('arrow-' + lastSection), 0.05, {bottom: "30px"})
						.to(document.getElementById('overlay-' + lastSection), 0.05, {height: "100%"})
						.to(document.getElementById('panel-' + lastSection), 0.05, {"border-bottom": "13px solid #ccc"})
						.to(document.getElementById('panel-' + lastSection), 0.05, {backgroundColor: "transparent"});
				}
				lastSection = num;
			}
		};
		...
		$scope.accordionPop = function (num) {
			TweenMax.to(document.getElementById('accordion-pop-' + num), 0.15, {ease: Power3.easeIn,height: "auto"});
			TweenMax.to(document.getElementById('accordion-pop-' + num), 0.25, {color: "#161616"});
			TweenMax.to(document.getElementById('plus-sign-' + num), 0.5, {transform: "rotate(315deg)"});

			if (lastAccordion !== "" && document.getElementById('accordion-pop-' + lastAccordion).style.height !== "0px") {
				TweenMax.to(document.getElementById('accordion-pop-' + lastAccordion), 0.05, {color: "transparent"});
				TweenMax.to(document.getElementById('accordion-pop-' + lastAccordion), 0.5, {ease: Power3.easeOut,height: 0});
				TweenMax.to(document.getElementById('plus-sign-' + lastAccordion), 0.5, {transform: "rotate(0deg)"});
			}
			lastAccordion = num;
		};
